...
// Creo un programma
	...
	// Posso scrivere il codice in un file a parte (.cl) e poi importarlo
	const char* source = {
...
/// CREATE MEMORY OBJECTS
	...
	size_t DATA_SIZE = 3;
	cl_mem buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(buf), NULL, NULL);
...
// Copio i dati dalla cpu alla gpu
	...
	clEnqueueWriteBuffer(queue, buffer, CL_FALSE, 0, sizeof(buf), buf, 0, NULL, NULL);
...
// Sposto i dati dalla gpu alla cpu
	...
	void* data1 = &dd;
	clEnqueueReadBuffer(queue, buffer, CL_TRUE, 0, sizeof(buf), &buf, 0, NULL, NULL);
	float* output = (float*)data1;
	printf("valore finale: %s", buf);
