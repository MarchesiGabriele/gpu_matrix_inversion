...
// CHECK DOCUMENTATION FOR KERNELS INFO
	...
	std::vector<float> matrix_inversion_FP32(std::vector<float> input_matrix, int matrix_order) {
		...
		try {
			...
			// Primo buffer per matrice augmentata
			...
			// Secondo buffer per matrice augmentata	
			buffers.push_back(cl::Buffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,  augmented_matrix.size() * sizeof(cl_float), augmented_matrix.data(), &operationResult));
			...
			std::vector<cl_float> m = std::vector<cl_float>(augmented_matrix.size(), 0);
...
/*
				if (flag)
					operationResult = commandQueue.enqueueReadBuffer(buffers[0], CL_TRUE, 0, augmented_matrix.size() * sizeof(cl_float), augmented_matrix.data(), NULL);
				else
					operationResult = commandQueue.enqueueReadBuffer(buffers[2], CL_TRUE, 0, augmented_matrix.size() * sizeof(cl_float), augmented_matrix.data(), NULL);
...
/*
				operationResult = commandQueue.enqueueReadBuffer(buffers[3], CL_TRUE, 0, numeroWorkgroups * sizeof(cl_float2), max_pivots.data(), NULL);
...
*/
			...
			if (((matrix_order - 1) % 2) == 0)
				operationResult = commandQueue.enqueueReadBuffer(buffers[2], CL_TRUE, 0, augmented_matrix.size() * sizeof(cl_float), m.data(), NULL);
			else
				operationResult = commandQueue.enqueueReadBuffer(buffers[0], CL_TRUE, 0, augmented_matrix.size() * sizeof(cl_float), m.data(), NULL);
